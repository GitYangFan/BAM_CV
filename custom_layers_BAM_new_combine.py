...
class model_combine(tf.keras.Model):
    def __init__(self, n_channels_main=16, n_channels2=16, data_layers=2, cov_layers=4, inner_channels=10, N_exp=3, N_heads=5,
        ...
        self.dense_layer = layer_dense(self.num_classes)

    def call(self, inputs, **kwargs):
        bam = self.BAM_layers(inputs)
        prob = self.dense_layer(bam)
        return prob
...
class model_attention_final(tf.keras.Model):
    ...
    def __init__(self, n_channels_main=16, n_channels2=16, data_layers=2, cov_layers=4, inner_channels=10, N_exp=3,
                 N_heads=5,
                 num_classes=7):
    ...
    def call(self, inputs, **kwargs):
        ...
        out_reshape = tf.reshape(out, shape=(
        -1, self.n_channels2, self.n_channels_main, self.n_channels_main))  # shape (N, 1, c, c)
...
class layer_dense2(tf.keras.layers.Layer):  # output the classes
    ...
    def call(self, inputs):
        inputs_flatten = self.flatten(inputs)
        # inputs_flatten = tf.reshape(inputs, shape=(-1, tf.shape(inputs)[1] * tf.shape(inputs)[2] * tf.shape(inputs)[3]))
        conv = self.linear_model(inputs_flatten)
        return conv
...
class layer_N_C_d_d_bilinear_attention_cov2cor_spd(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        def init2(shape, dtype=ddtype):
            return tf.cast(
                l1_constraint_columns(tf.abs(tf.random.normal(shape, 0, 1 / tf.sqrt(tf.cast(shape[0], tf.float32))))),
                dtype)
